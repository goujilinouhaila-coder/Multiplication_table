...
def test_circle():
    ...
    background = "white"
    outline = "black"
    assert type(circle(cnv, center, radius, state, background, outline)) == tkinter.Canvas
...
def test_dot():
    ...
    color_name = "red"
    assert type(dot(cnv, graphe, radius, center, color_graphe, color_name)) == tkinter.Canvas
...
def test_name_vertices():
    ...
    radius = 50
    color = "red"
    assert type(name_vertices(cnv, radius, graphe, center, color)) == tkinter.Canvas
