...
def user_IU():
    ...
    cnv.pack(side="left")
    show_update()
    table_cursor = Scale(root, label="Table of ...", font="Arial 15 bold",
                        orient="horizontal", command=table, from_=2, to=450,
                        length=250, resolution=0.01)
    table_cursor.pack(pady=15)
    peak_cursor = Scale(root, label="Number of peaks", font="Arial 15 bold",
                        orient="horizontal", command=peak, from_=2,
                        to=200, length=250)
